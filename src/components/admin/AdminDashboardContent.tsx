...
const AdminDashboardContent = () => {
  ...
  const handleToggleCotisations = async (profileId: string, currentValue: boolean) => {
    ...
    toast({ title: newValue ? "Cotisations marquées à jour" : "Cotisations non à jour" });
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground">Gestion des partenaires et du réseau</p>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <Card><CardContent className="py-4 text-center"><div className="text-3xl font-bold text-amber-500">{pending.length}</div><p className="text-sm text-muted-foreground">En attente</p></CardContent></Card>
            <Card><CardContent className="py-4 text-center"><div className="text-3xl font-bold text-green-500">{approved.length}</div><p className="text-sm text-muted-foreground">Approuvés</p></CardContent></Card>
            <Card><CardContent className="py-4 text-center"><div className="text-3xl font-bold text-blue-500">{credited.length}</div><p className="text-sm text-muted-foreground">Crédités</p></CardContent></Card>
            <Card><CardContent className="py-4 text-center"><div className="text-3xl font-bold text-destructive">{rejected.length}</div><p className="text-sm text-muted-foreground">Refusés</p></CardContent></Card>
          </div>

          {pending.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold flex items-center gap-2"><Clock className="w-5 h-5 text-amber-500" />Demandes en attente ({pending.length})</h2>
              {pending.map((p) => <PartnerCard key={p.id} partner={p} updating={updating} onApprove={() => updateStatus(p.id, "approved")} onReject={() => updateStatus(p.id, "rejected")} onDownloadDoc={handleDownloadDoc} onValidateDoc={handleValidateDoc} onToggleCotisations={handleToggleCotisations} />)}
            </div>
          )}
          {approved.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-green-500" />Partenaires approuvés ({approved.length})</h2>
              {approved.map((p) => <PartnerCard key={p.id} partner={p} updating={updating} onApprove={() => updateStatus(p.id, "approved")} onReject={() => updateStatus(p.id, "rejected")} onDownloadDoc={handleDownloadDoc} onValidateDoc={handleValidateDoc} onToggleCotisations={handleToggleCotisations} />)}
            </div>
          )}
          {credited.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-blue-500" />Partenaires crédités ({credited.length})</h2>
              {credited.map((p) => <PartnerCard key={p.id} partner={p} updating={updating} onApprove={() => updateStatus(p.id, "approved")} onReject={() => updateStatus(p.id, "rejected")} onDownloadDoc={handleDownloadDoc} onValidateDoc={handleValidateDoc} onToggleCotisations={handleToggleCotisations} />)}
            </div>
          )}
          {rejected.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold flex items-center gap-2"><XCircle className="w-5 h-5 text-destructive" />Partenaires refusés ({rejected.length})</h2>
              {rejected.map((p) => <PartnerCard key={p.id} partner={p} updating={updating} onApprove={() => updateStatus(p.id, "approved")} onReject={() => updateStatus(p.id, "rejected")} onDownloadDoc={handleDownloadDoc} onValidateDoc={handleValidateDoc} onToggleCotisations={handleToggleCotisations} />)}
            </div>
          )}
          {partners.length === 0 && (
            <Card><CardContent className="py-12 text-center text-muted-foreground">Aucune demande de partenaire pour le moment.</CardContent></Card>
          )}
        </div>
      </main>
      <Footer />
      ...
      <CardContent className="py-5">
        <div className="flex items-start justify-between gap-4">
                ...
                <div className="space-y-1.5">
                      ...
                      <div key={docType} className="flex items-center gap-2">
                        ...
                        {doc && (
                          <div className="flex items-center gap-1">
                            <Button variant="ghost" size="sm" className="h-6 px-2 text-[10px]" onClick={() => onDownloadDoc(doc.file_path, doc.file_name)}>
                              <Download className="w-3 h-3 mr-1" />Télécharger
                            </Button>
                            {!doc.validated ? (
                              <Button variant="outline" size="sm" className="h-6 px-2 text-[10px] border-green-500 text-green-600 hover:bg-green-50" onClick={() => onValidateDoc(doc.id, true)}>
                                <CheckCircle2 className="w-3 h-3 mr-1" />Valider
                              </Button>
                            ) : (
                              <Button variant="outline" size="sm" className="h-6 px-2 text-[10px] border-amber-500 text-amber-600 hover:bg-amber-50" onClick={() => onValidateDoc(doc.id, false)}>
                                <XCircle className="w-3 h-3 mr-1" />Invalider
                              </Button>
                            )}
                          </div>
                        )}
                        {!doc && <span className="text-[10px] text-muted-foreground italic">Non envoyé</span>}
                      </div>
                    );
                  })}
                  <div className="flex items-center gap-2">
                    <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-full border ${partner.cotisations_a_jour ? "bg-green-500/10 text-green-700 border-green-500/20" : "bg-amber-500/10 text-amber-600 border-amber-500/20"}`}>
                      <CreditCard className="w-3.5 h-3.5" />Cotisations
                      {partner.cotisations_a_jour ? <CheckCircle2 className="w-2.5 h-2.5" /> : <XCircle className="w-2.5 h-2.5" />}
                    </span>
                    <Button variant="outline" size="sm" className={`h-6 px-2 text-[10px] ${partner.cotisations_a_jour ? "border-amber-500 text-amber-600 hover:bg-amber-50" : "border-green-500 text-green-600 hover:bg-green-50"}`} onClick={() => onToggleCotisations(partner.id, partner.cotisations_a_jour)}>
          ...
          <div className="flex gap-2 shrink-0">
            {partner.status !== "approved" && (
              <Button size="sm" onClick={onApprove} disabled={updating === partner.id}>
                {updating === partner.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-1" />}Approuver
              </Button>
            )}
            {partner.status !== "rejected" && (
              <Button size="sm" variant="destructive" onClick={onReject} disabled={updating === partner.id}>
                {updating === partner.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4 mr-1" />}Refuser
              </Button>
            )}
          </div>
        </div>
